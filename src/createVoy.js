...
import createAIImportGuard from './createAIImportGuard.js';

export function createVoy(userConfig = {}) {
  const config = {
    ...defaultConfig,
    ...userConfig,
  ...
  const pasteHandler = createPasteHandler({ state, config, maybeReport, report });
  ...
  const aiImportGuard = createAIImportGuard({ report });
  ...
  const start = () => {
    ...
    registerListener(document, 'input', aiImportGuard, undefined, listeners);
